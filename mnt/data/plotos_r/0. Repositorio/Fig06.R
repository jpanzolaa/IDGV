...
boxplot_inset <- ggplot(df, aes(x = Sujeto, y = Indice, fill = Sujeto)) +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 0, hjust = 0.5, color = "blue", size = 12),
...
main_plot <- ggplot(df, aes(x = Hora, y = Indice, fill = Sujeto)) +
  ...
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1, vjust = 1, size = 12, color = "blue"),
    axis.text.y = element_text(color = "blue", size = 12),
...
# Guardar el gráfico en PNG con resolución 900 x 550 píxeles
ggsave("Fig06.png", plot = final_plot, width = 9, height = 5.5, units = "in", dpi = 100)

# Guardar el gráfico en PDF
ggsave("Fig06.pdf", plot = final_plot, width = 9, height = 5.5, units = "in")

# Mensaje de confirmación
cat("Gráfico guardado como:\n- Fig06.png (900x550 px)\n- Fig06.pdf\n")